class Iterator:
    ...
    def __next__(self):
        ...
        # приращение "указателя" на "шаг"
        self.pointer += self.step
        # условие когда шаг положительные и идёт приращение на увеличение
        if self.step > 0 and self.pointer < self.stop:
            return self.pointer
        # условие когда шаг отрицательный и идёт урезание так как шаг минусует и так будет до тех пор пока не станет стопом
        if self.step < 0 and self.pointer > self.stop:
            return self.pointer
